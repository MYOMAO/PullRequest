   ...
   // particle->SetDecayTable(NULL);   
...
#endif
    {
     ...
     //   particle->SetDecayTable(NULL);   
        ...
        G4DecayTable   *decaytable = particle->GetDecayTable();
	std::cout <<  __PRETTY_FUNCTION__ << " ROCK: decay->SetExtDecayer(extDecayer); " << "  i = " << i << "   aParticleIterator->value()->GetParticleName() = " << aParticleIterator->value()->GetParticleName() << "   decaytable = " << decaytable   <<  std::endl;
        decayer_used++;
      }
    }
  }
